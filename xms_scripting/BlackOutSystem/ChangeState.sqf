...
_currentState = _unit getVariable "xms_XMed_var_BOS_STATE";

switch (_state) do
{
    case -1: {
        [_text] call xms_XMed_fnc_BOS_ChangeDisplayText;
        RETURN(true);
    };
    case 0: {
        if (_currentState > 1 && {!_override || ([_unit] call xms_XMed_fnc_BOS_CheckConditions)}) then
        {
            RETURN(false);
        }
        else
        {
            _unit setVariable ["xms_XMed_var_BOS_STATE", _state, true];
    ...
    case 1: {
        ...
        else
        {
            ...
            _unit setVariable ["xms_XMed_var_BOS_INFO", _tmpArr];
            
            [_unit] call xms_XMed_fnc_BOS_BlackOutUnit;
            [_text] call xms_XMed_fnc_BOS_ChangeDisplayText;
    ...
    case 2: {
        if (_currentState > 2 && {!_override || ([_unit] call xms_XMed_fnc_BOS_CheckConditions)}) then
        {
            RETURN(false);
        }
        else
